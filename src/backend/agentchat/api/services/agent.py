...
class AgentService:
    ...
    def _to_dict_list(results):
        ...
        """
        items = [res.to_dict() for res in results] if results else []
        try:
                ...
                for item in items:
                    ...
                    if isinstance(logo, str) and (logo.startswith("http://") or logo.startswith("https://")):
                        ...
                        if base_netloc and parsed.netloc == base_netloc:
                            ...
                            item["logo_url"] = storage_client.sign_url_for_get(object_name)
                    elif isinstance(logo, str):
                        item["logo_url"] = storage_client.sign_url_for_get(logo)
        except Exception:
            pass
        return items
