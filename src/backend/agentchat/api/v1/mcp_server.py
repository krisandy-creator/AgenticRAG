...
async def create_mcp_server(
    ...
    try:
        ...
        try:
            ...
            tool_names = [t for t in tools_name_str if isinstance(t, str)]
            name_hint = "mcp_tools_agent"
            if any("weather" in (t or "").lower() for t in tool_names):
                name_hint = "weather_tools_agent"
            structured_response = MCPResponseFormat(
                mcp_as_tool_name=name_hint,
                description="子智能体用于调用该MCP服务下的多个工具来完成用户请求，遇到需要该服务能力的场景时应整合用户问题后一次性发起调用。"
            )
        ...
        await MCPService.create_mcp_server(
            ...
            logo_url=normalize_object_storage_value(req.logo_url),
...
async def update_mcp_server(
    ...
    try:
        ...
        update_data = {}
        normalized_logo_url = normalize_object_storage_value(req.logo_url) if req.logo_url is not None else None
        if req.imported_config and req.imported_config != mcp_server["imported_config"]:
            ...
            if req.logo_url is not None:
                update_data["logo_url"] = normalized_logo_url
