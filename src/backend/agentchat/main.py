...
async def init_config():
    ...
    # 企业 RAG 原型只初始化知识库闭环所需表和演示账号
    from agentchat.infrastructure.db.bootstrap import init_enterprise_rag_database

    await init_enterprise_rag_database()

def print_logo():
    from pyfiglet import Figlet

    f = Figlet(font="slant")
    print(f.renderText("OmniRAG"))
...
def create_app():
    app = FastAPI(
        title=app_settings.server.get("project_name", "Enterprise RAG Knowledge Base"),
